...
internal sealed class TransientRequestExceptionRequestHandler<TRequest, TException> : RequestExceptionRequestHandler
    where TRequest : IRequest
    where TException : Exception
{
    public override Task HandleAsync<TMethodRequest>(TMethodRequest request, Exception exception, RequestExceptionHandlerState state, IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
    ...
    private IRequestExceptionHandler<TRequest, TException>? GetHandler(IServiceProvider serviceProvider) =>
        serviceProvider.GetService<IRequestExceptionHandler<TRequest, TException>>();
}